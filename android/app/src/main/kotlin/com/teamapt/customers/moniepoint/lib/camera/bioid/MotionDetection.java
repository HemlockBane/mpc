...
public class MotionDetection {
    ...
    private final Logger log = Logger.getLogger(MotionDetection.class.getName());
     ...
     */
    @WorkerThread
    public void createTemplate(@NonNull Yuv420Image first) {

        log.info("creating template for motion detection");
...
//        log.stopStopwatch(stopwatchSessionId);
     ...
     */
    @WorkerThread
    public boolean detect(@NonNull Yuv420Image current) {
        if (templateBuffer == null) {
            throw new IllegalStateException("missing template");
        }

        log.info("motion detection algorithm");
        ...
        log.info( String.format(Locale.ENGLISH, "detected motion of %.2f%%", movementPercentage));
