...
abstract class TransactionDao extends MoniepointDao<AccountTransaction> {
  ...
  @Query(
      "SELECT * FROM account_transactions WHERE (transactionDate BETWEEN :startDate AND :endDate) AND (transactionChannel IN (:channels) OR transactionChannel is null) AND type IN (:transactionTypes) ORDER BY transactionDate DESC LIMIT :limit OFFSET :myOffset"
  )
